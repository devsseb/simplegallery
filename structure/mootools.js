...
try{
	...
	console = function(args) {
		if (!this.workspace) {
			this.total = 0;
			this.workspace = new Element('div', {styles: {
				position: 'fixed',
				left: 0,
				bottom: 0,
				backgroundColor: '#eaecff',
				border: [1, 'solid', '#0f4f8b'],
				fontFamily: 'monospace',
				width: '100%',
				zIndex: 999999
			}}).grab(new Element('div[html=Console javascript]', {styles: {fontWeight: 'bold'}})).inject(document.body);
		}
		var message = '';
		for (var i = 0; i < args.length; i++) {
			message+= (i > 0 ? '<span>&nbsp;&nbsp;&nbsp;&nbsp;</span>' : '') + args[i];
		}
		var div = new Element('div', {html: message, styles: {
			padding: [0, 5, 0, 15],
			backgroundColor: this.total++%2 ? 'transparent' : '#dcdde5',
			height: 14,
			position: 'relative'
		}}).inject(this.workspace);
		
		if (div.getScrollSize().y > 14) {
			new Element('a[html=&#x25B6]', {styles: {
				display: 'block',
				width: 8,
				height: 8,
				lineHeight: 8,
